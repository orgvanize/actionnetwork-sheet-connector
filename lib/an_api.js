...
const getEventAttendances = async (eventId, href = null, attendances = []) => {
    const result = await runApiCall('GET', href || `events/${eventId}/attendances`);
    attendances = attendances.concat(result._embedded['osdi:attendances']);
    if(result._links && result._links['next'] && result._links['next'].href != href) {
        attendances = await getEventAttendances(eventId, result._links['next'].href.replace(process.env.AN_API_BASE, ''), attendances);
    }
    return attendances;
}
